...
const Skills = () => {
  ...
  useEffect(() => {
    ...
    const io= new IntersectionObserver(([entry])=>{
      setSctive(entry.isIntersecting&&entry.intersectionRatio>0.1)
    },
  {threshold:[0.1]})
  io.observe(el)
...
useEffect(() => {
  ...
  return (
    ...
    <section id="skills" ref={sectionRef} className='h-1/2 w-full pb-8 flex flex-col items-center justify-center relative bg-black text-white overflow-hidden'>
      ...
      transition={{duration:0.5,delay:0.1}}>My Skills

      </motion.h2>
      <motion.p className='mt-2 mb-8 text-white/90 text-base sm:text-lg z-10'
      initial={{opacity:0,y:-10}}
      whileInView={{opacity:1,y:0}}
      transition={{duration:0.5,delay:0.1}}> Modern Application || Moder technologies</motion.p>
        ...
        style={{x,whiteSpace:"nowrap", willChange:"transform"}}>
          {repeated.map((s, i) => (
            <div key={i} className='flex flex-col items-center gap-2 min-w-[120px]'
              aria-label={s.name}
              title={s.name}>
              <span className='hover:scale-125 transition-transform duration-300 mt-3'>
                {s.icon}
              </span>
